...
void draw_map(FrameBuffer &fb, const std::vector<Sprite> &sprites, const Texture &tex_walls, const Map &map, const size_t cell_w, const size_t cell_h) {
	for (size_t j=0; j<map.h; j++) {
		for (size_t i=0; i<map.w; i++) {
			...
			fb.draw_rectangle(rect_x, rect_y, cell_w, cell_h, tex_walls.get(0, 0, texid)); // color is taken from upper left pixel of the texture #texid
		}
	}
}
...
void draw_sprite(FrameBuffer &fb, const Sprite &sprite, const std::vector<float> &depth_buffer, const Player &player, const Texture &tex_sprites) {
	...
	int h_offset = (sprite_dir-player.a)*(fb.w/2)+(fb.w/2)/2-tex_sprites.size/2; //3d view takes half of fb
...
void render(FrameBuffer &fb, const GameState &gs) {
	...
	const size_t cell_w = fb.w/(map.w*2); // size of one map cell on screen
	...
	for (size_t i=0; i<fb.w/2; i++) { // draw visibility cone and the 3D view
		...
		for (float t=0; t<20; t+=.01) { // ray marching loop
			...
			fb.set_pixel(x*cell_w, y*cell_h, pack_color(160, 160, 160)); // draws visibiliy cone
