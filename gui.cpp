...
int main() {
	...
	GameState gs{ Map(),								// game map
                  ...
                  { {3.523, 3.812, 2, 0},               // monsters lists
                    ...
                    {5.323, 5.365, 1, 0},
                    {4.123, 10.76, 1, 0} },
                  Texture("texturesfolder/walltext.png"),           // textures for the walls
                  Texture("texturesfolder/monsters.png") };         // textures for the monsters
	...
	if (SDL_CreateWindowAndRenderer(fb.w, fb.h, SDL_WINDOW_SHOWN | SDL_WINDOW_INPUT_FOCUS, &window, &renderer)) {
		std::cerr << "Couldnt create window and renderer: " << SDL_GetError() << std::endl;
		return -1;
	}
	...
	while (1) {
		...
		for (size_t i=0; i<gs.monsters.size(); i++) { // update the distances from the player to each sprite
			...
			std::sort(gs.monsters.begin(), gs.monsters.end()); // sort it from furthest to closest
		}
